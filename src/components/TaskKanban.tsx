...
const TaskKanban: React.FC<TaskKanbanProps> = ({ theme, storyId }) => {
    ...
    const renderTask = (task: Task) => (
        <div key={task.id} style={{ border: '1px solid #ccc', margin: 8, padding: 8, background: theme === 'dark' ? '#222' : '#fff' }}>
            ...
            <div>
                <button onClick={() => setSelectedTask(task)} className={`btn ${theme === 'dark' ? 'btn-outline-secondary' : 'btn-outline-dark'}`}>Details</button>
                <button onClick={() => handleDelete(task.id)} className={`btn ${theme === 'dark' ? 'btn-outline-danger' : 'btn-outline-secondary'}`}>Delete</button>
            </div>
        </div>
    );
        ...
        <div style={{ display: 'flex', gap: 16 }}>
            ...
            {selectedTask && (
                <div style={{ position: 'fixed', top: 40, right: 40, background: theme === 'dark' ? '#333' : '#fff', color: theme === 'dark' ? '#fff' : '#222', padding: 24, borderRadius: 8, zIndex: 1000, border: theme === 'light' ? '2px solid #222' : 'none' }}>
                    ...
                    <div>
                        <button onClick={() => handleStateChange(selectedTask.id, 'todo')} className={`btn ${theme === 'dark' ? 'btn-outline-secondary' : 'btn-outline-dark'}`}>Set To Do</button>
                        <button onClick={() => handleStateChange(selectedTask.id, 'doing')} className={`btn ${theme === 'dark' ? 'btn-outline-secondary' : 'btn-outline-dark'}`}>Set Doing</button>
                        <button onClick={() => handleStateChange(selectedTask.id, 'done')} className={`btn ${theme === 'dark' ? 'btn-outline-secondary' : 'btn-outline-dark'}`}>Set Done</button>
                        <button onClick={() => setSelectedTask(null)} className={`btn ${theme === 'dark' ? 'btn-outline-danger' : 'btn-outline-secondary'}`}>Close</button>
                    </div>
                </div>
            )}
        </div>
